<?php
function easyweb_webnus_parallax($attributes, $content) {
	...
	$out = '</div></section><section '.$section_id.' class="parallax-sec '.$is_dark.' blox ' . $class .' '.$row_pattern . '" style="' . $padding_style . $background_style . $bgcolor . $background_size . $height_style . '" data-stellar-background-ratio="'.$speed. '"><div class="max-overlay" style="'.  $color_overlay .'"></div><div data-stellar-ratio="1" class="wpb_row vc_row-fluid "><div class="container">';
	$out .= do_shortcode($content);
	$out .= '</div></div></section><section class="container"><div class="row-wrapper-x">';
	return $out;
}
add_shortcode('parallax', 'easyweb_webnus_parallax');
?>